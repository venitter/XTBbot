...
def procTickExample(msg):
    dt = datetime.datetime.fromtimestamp(msg['data']['timestamp'] / 1000).strftime('%Y-%m-%d %H:%M:%S')
    ask = msg['data']['ask']
    bid = msg['data']['bid']
    # print("TICK [", dt, "]: ASK [", ask, "] / BID [", bid, "]")
...
def main():
    ...
    sclient = APIStreamClient(ssId=ssid, tickFun=procTickExample, tradeFun=procTradeExample,
                              ...
                              candlesFun=procCandlesExample)
    sclient.subscribePrice("EURUSD")
    #sclient.subscribeCandles("EURUSD")

    #timenow = int(time.time())*1000
    #print(timenow)
    #resp = client.commandExecute("getChartRangeRequest", dict(info=dict(start=timenow-60*1000*25,
    #                                                                    end=timenow-60*1000,
    #                                                                    period=1,
    #                                                                    symbol="BITCOIN")))
    #print(resp)
